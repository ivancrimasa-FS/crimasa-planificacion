...
export async function leerExcel(file: File): Promise<Importado> {
  ...
  if (hojas.some((h) => /^Obras_CyM$/i.test(h))) {
    leerDatosMaestros(wb, XLSX, out);
  }
...
/* ------------------- DATOS_PLANIFICACION_ANUAL (CyM) ------------------ */

/**
 * Libro de datos maestros con hojas normalizadas (Obras_CyM, Personal_CyM,
 * Vehiculos_CyM y JO_CyM). Es el archivo de origen: siempre se leen las hojas
 * CyM, que llevan CRIMASA y MAC juntos.
 */
function leerDatosMaestros(wb: any, XLSX: any, out: Importado) {
  const hoja = (nombre: string): any[] => {
    const real = Object.keys(wb.Sheets).find((h) => h.toLowerCase() === nombre.toLowerCase());
    if (!real) return [];
    return XLSX.utils.sheet_to_json(wb.Sheets[real], { defval: null }) as any[];
  };

  const campo = (fila: any, ...nombres: string[]) => {
    for (const n of nombres) {
      for (const k of Object.keys(fila)) {
        if (k.trim().toLowerCase() === n.toLowerCase()) return fila[k];
      }
    }
    return null;
  };
  ...
  const codigosSinJefe = new Set<string>();
  for (const fila of obras) {
    ...
    const datos = {
      code,
      name: String(campo(fila, "nombre") || code).trim(),
  ...
  // --- Personal ---
  // Los que tienen categoría "Jefe de obra" no entran en la lista de operarios:
  // son jefes, y ya están arriba.
  const personal = hoja("Personal_CyM");
  for (const fila of personal) {
    const nombre = String(campo(fila, "nombre_completo", "nombre") || "").trim();
    if (nombre.length < 4) continue;
    const categoria = String(campo(fila, "categoria") || "").trim();
    if (/jefe de obra/i.test(categoria)) continue;
  ...
  // --- Vehículos ---
  const vehiculos = hoja("Vehiculos_CyM");
  for (const fila of vehiculos) {
    const plate = String(campo(fila, "matricula") || "").trim().toUpperCase().replace(/\s+/g, "");
    if (!plate) continue;
    if (out.vehicles.some((v) => v.plate === plate)) continue;
    const desc = [campo(fila, "marca"), campo(fila, "modelo"), campo(fila, "Propiedad", "propiedad")]
      .map((x) => String(x || "").trim())
      .filter(Boolean)
      .join(" · ");
    out.vehicles.push({ id: slug("vh", plate), plate, description: desc, active: true });
  }

  out.resumen.push(
    "Datos maestros (hojas CyM): " +
      out.managers.length +
      " jefes de obra, " +
      obras.length +
      " obras, " +
